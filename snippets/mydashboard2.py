...
@st.cache(persist=True)
def getmedata():
    url =  "/Users/lilit/PycharmProjects/patima_project/bread-recipe-generator/Groceries_dataset.csv"  #'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv'

    df = pd.read_csv(url, delimiter=',', header='infer')
    print(df.head())
    df.rename(index=lambda x: df.at[x, 'itemDescription'], inplace=True)
    dft = df.loc[df['itemDescription'].isnull()]
    dft = dft.transpose()
    dft = dft.drop(['itemDescription', 'Member_number'])
    dft.index = pd.to_datetime(dft.index)
    return(dft, df)
...
countrylist1 = ['United Kingdom', 'US', 'India']
